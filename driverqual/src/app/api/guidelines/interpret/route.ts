...
import { interpretGuideline } from '@/server/interpretation';
...
export async function POST(request: Request) {
  ...
  if (files.length === 0) {
    return NextResponse.json({ error: 'Attach the guideline document.' }, { status: 400 });
  }
  ...
  const result = await interpretGuideline(await getDb(), encoded);
