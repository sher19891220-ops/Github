...
import { ENTITY_XTRACK_ID, ensureBaseFixtures, uniqueText } from './helpers';
...
describe('createDocument + getDocumentSummary/getDocumentRows', () => {
  ...
  it('stores binary content without attempting to parse it, and says why', async () => {
    ...
    const summary = await getDocumentSummary(created.documentId);
    expect(summary?.parseStatus).toBe('failed');
    expect(summary?.parseError).toMatch(/no text-extraction step/);
  });
